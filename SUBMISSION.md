...
> **Live AI is configured:** the public link runs real inference on **NVIDIA NIM**
> (OpenAI-compatible, Llama 3.1 8B) — chosen for fast, reliable streaming so the
> full 5-agent pipeline + 108-persona council completes live in well under a
> minute. The link ALSO works with **zero configuration** in demo mode, and any
> live run that hits a quota/latency limit degrades to a clearly-labeled
> deterministic demo, so a recording can never break.
...
**AI Tools Used (≤800):**
> NVIDIA NIM (build.nvidia.com, OpenAI-compatible, Llama 3.1 8B) — the inference provider powering the LIVE public demo, chosen for fast reliable streaming; model swappable via OPENROUTER_MODEL. Google Gemini via @google/genai and Groq — supported alternate providers (Gemini adds native Google Search grounding). Claude Code by Anthropic — AI pair-programmer used to harden, secure, refactor, and deploy. Vercel — hosting (serverless functions + static build). The app also ships a built-in deterministic demo mode so it runs with no paid service.
...
> *How it was built.* React 19 + Vite + Tailwind v4 frontend; a shared agent library streams each step; deployed as Vercel serverless functions with a pluggable provider (live on NVIDIA NIM; Gemini / Groq / any OpenAI-compatible endpoint / a built-in mock) so it always runs.
...
> *(Agent 3 table.)* "Agent three **simulates** each option over five years — the vulnerability-weighted plan reaches far more high-risk residents than central placement." *(Read the actual on-screen numbers; live figures vary by run. The seeded demo mode shows ~82% vs ~38% if you want a fixed talking point.)*
...
- **0:15–0:35 — One line.** "CIVICTAS advises; a human decides. Live, on NVIDIA NIM." Show the engine badge.
...
```
   ...
   └────────────────────┘     │  │  Agent 4 Equity Audit      │     │ Exportable record      │
                              │  └─ Agent 5 Brief             │     └───────────┬────────────┘
   key stays SERVER-SIDE ─────┤   NVIDIA NIM · Llama 3.1 8B   │                 │
...
- **NVIDIA NIM (Llama 3.1 8B):** fast streaming so the 5-agent reasoning is visible live and the whole pipeline finishes in well under a minute; provider/model is swappable (Gemini adds native grounding; a larger model trades latency for depth).
