...
/**
 * Plain-text fallback for the decision brief. No jsPDF, no eval/Function, so it
 * works under the strict production CSP even if the PDF path is blocked. Triggered
 * automatically if downloadDecisionBrief throws.
 */
export function downloadTextBrief(details: Parameters<typeof downloadDecisionBrief>[0]): void {
  const a = details.fullAgentOutputs;
  const text = [
    "CIVICTAS — DECISION BRIEF",
    "AI advised · a human decided",
    "=".repeat(60),
    `Generated: ${details.timestamp}`,
    `Category: ${details.category}`,
    `Budget: $${details.budget}   Sites: ${details.sites}`,
    `Equity goal: ${details.equityGoal}`,
    "",
    `HUMAN DECISION: ${details.humanDecision}`,
    `Chosen option: ${details.chosenOption}`,
    `Confidence: ${details.confidence}`,
    "",
    "HUMAN RATIONALE",
    details.humanRationale,
    "",
    "SITUATION",
    details.situation,
    "",
    "AI RECOMMENDATION (Agent 5 — proposal only)",
    details.aiRecommendation,
    ...(details.riskNote ? ["", "EQUITY & RISK NOTE (Agent 4)", details.riskNote] : []),
    ...(a
      ? ["", "FULL ADVISORY LOG", "— Framing —", a.step1, "— Evidence —", a.step2, "— Simulation —", a.step3, "— Equity & Risk Audit —", a.step4, "— Brief —", a.step5]
      : []),
    "",
    "=".repeat(60),
    "Generated by CIVICTAS. AI advises; a human decides.",
  ].join("\n");

  const sanitized = details.category.toLowerCase().replace(/[^a-z0-9]+/g, "_").slice(0, 30);
  const blob = new Blob([text], { type: "text/plain;charset=utf-8" });
  const url = URL.createObjectURL(blob);
  const link = document.createElement("a");
  link.href = url;
  link.download = `civictas_decision_brief_${sanitized}.txt`;
